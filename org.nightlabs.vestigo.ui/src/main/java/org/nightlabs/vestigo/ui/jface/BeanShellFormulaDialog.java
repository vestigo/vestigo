...
public class BeanShellFormulaDialog extends TitleAreaDialog implements FormulaDialog<BeanShellFormula>
{
	...
	private BeanShellFormula value;
	private Text text;

	public BeanShellFormulaDialog(Shell parentShell) {
		super(parentShell);
	}
		...
		if (text != null && !text.isDisposed()) {
			if (value == null || !value.getFormulaText().equals(text.getText())) {
				if (text.getText().isEmpty())
					...
					value = new BeanShellFormula(text.getText());
			}
		}
	...
	protected Control createContents(Composite parent) {
		...
		setTitleImage(VestigoUIPlugin.getDefault().getImage(BeanShellFormulaDialog.class, "title", AbstractVestigoUIPlugin.IMAGE_SIZE_75x70)); //$NON-NLS-1$
		setTitle(title);
		setMessage(Messages.getString("BeanShellFormulaDialog.message")); //$NON-NLS-1$
